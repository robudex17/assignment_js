...
module.exports = class Inbound {
    ...
    static callSummary(date) {
        ...
        return database.execute(sql).then((rows,fields) => {
            ...
            for (let i=0;i<rows.length;i++){
                await this.callDuration(getDate,rows[i].extension).then(summary => {
   ...
   static callDuration(date,extension) {
        ...
        return database.execute(sql).then((rows,fields) => {
            ...
            let totalduration = [];
            for (let i=0; i<rows.length; i++) {             
                 totalduration[i] = await  (this.stringToTimeStamp(rows[i].EndTimeStamp).getTime - this.stringToTimeStamp(rows[i].StartTimeStamp).getTime)
            }
    ...
    static getAgentCallDetails(extension, date) {
       ...
       const  sql = `SELECT * FROM  inbound_callstatus WHERE WhoAnsweredCall=${extension} AND getDate='${date}' `
        return database.execute(sql).then((rows, fields) => {
            ...
            const getCurrentRows = [...rows]
            for(let i=0; i<rows.length; i++){
              totalduration = await  (this.stringToTimeStamp(rows[i].EndTimeStamp).getTime - this.stringToTimeStamp(rows[i].StartTimeStamp).getTime)
              ...
              rows[i].startTime = this.stringToTimeStamp(rows[i].StartTimeStamp).toLocaleTimeString
              rows[i].endTime = this.stringToTimeStamp(rows[i].EndTimeStamp).toLocaleTimeString
            }
   ...
   static stringToTimeStamp(datestring){
        const objectTime = {}
